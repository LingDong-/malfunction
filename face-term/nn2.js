...
var NN = {}
NN.CLASSES = new Array(95).fill(0).map((x,i)=>String.fromCharCode(32+i)),
NN.IMAGE_W = 28,
NN.IMAGE_H = 28,
NN.IMAGE_SIZE = NN.IMAGE_H * NN.IMAGE_W;
NN.NUM_CLASSES = 95;
NN.NUM_DATASET_ELEMENTS = 23750;
NN.NUM_TRAIN_ELEMENTS = 20000;
NN.NUM_TEST_ELEMENTS = NN.NUM_DATASET_ELEMENTS - NN.NUM_TRAIN_ELEMENTS;
NN.IMAGES_SPRITE_PATH ='data/ascii_images.png';
NN.LABELS_PATH ='data/ascii_labels_uint8';
NN.EPOCHS = 100;
NN.model = undefined;
NN.data = undefined;
NN.DESCRIPTION = `ASCII`
...
NN.loadModel = async function(){
  NN.model = await tf.loadLayersModel('models/tfjs-model-5026090469481548.json');
}
...
NN.train = async function train() {
  ...
  function evaluateTest(epoch){
    ...
    ctx_g.fillStyle = "red";
    ctx_g.fillRect(canv_g.width * epoch/NN.EPOCHS, canv_g.height*(1-testAccPercent), 3, 3);
  }
...
NN.predictRaster = function(canvas){
...
NN.predictBatch = function(arr, n){
  const xs = tf.tensor4d(arr, [n, NN.IMAGE_H, NN.IMAGE_W, 1]);
  return tf.tidy(() => {
    
    let out = new Array(n).fill("");
  
    const output = NN.model.predict(xs);
    var probdist = output.dataSync();
    // console.log(probdist);
    for (let i = 0; i < n; i++){
      let m = 0;
      let am = 0;
      for (var j = 0; j < NN.NUM_CLASSES; j++){
        let v = probdist[i*NN.NUM_CLASSES+j];
        if (v > m){
          m = v;
          am = j;
        }
...
NN.embed = function(arr, n){
  if (!NN.embedder){
    const embeddingLayer = NN.model.layers[NN.model.layers.length - 2]; // second to last layer
    NN.embedder = tf.model({inputs: NN.model.inputs, outputs: embeddingLayer.output});
  }
  return tf.tidy(() => {
    const xs = tf.tensor4d(arr, [n, NN.IMAGE_H, NN.IMAGE_W, 1]);
    const output = NN.embedder.predict(xs);
    return output;
  });
}

NN.closest = function(arr,n){
  
  if (!NN.trainEmbeddings){
    let m = 1000;
    NN.trainEmbeddings = NN.embed(NN.data.trainImages.slice(0,m*NN.IMAGE_SIZE),m);
  }
  let {trainEmbeddings} = NN;

  return tf.tidy(()=>{
    let queryEmbeddings = NN.embed(arr,n);
    const T = tf.div(trainEmbeddings, trainEmbeddings.norm('euclidean', -1, true));
    const Q = tf.div(queryEmbeddings, queryEmbeddings.norm('euclidean', -1, true));
    const sims = Q.matMul(T.transpose()); // [M, N]
    const { indices } = tf.topk(sims, 1); // highest cosine similarity
